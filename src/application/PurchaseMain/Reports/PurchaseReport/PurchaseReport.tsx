...
export default function PurchaseReport() {
  ...
  const headers: (keyof RowData)[] = ['#', 'Invoice Number', 'Purchase Date', 'Supplier ID', 'Supplier Name', 'Invoice Total(₹)', 'Paid Payment(₹)', 'Due Amount(₹)', 'Due Days'];
  ...
  const handleSubmit = () => {
    const filtered = data.filter((row) => {
      if (category && row['Invoice Number'] !== category) return false;
      if (expireDate && new Date(row['Supplier Name']) < expireDate) return false;
      return true;
    });
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between p-4">
        <div className="flex flex-col md:flex-row items-center md:space-x-2">
          <span className="text-xl font-bold">Purchase Report</span>
        </div>
        <div className='flex'>
          ...
          <div className="flex items-center space-x-1 cursor-pointer text-black-500"
            onClick={() => navigate('/purchase/reports/purchasereport')}>
            <span>PurcahseReport</span>
          </div>
        </div>
      </div>
      ...
      <div className="m-2 p-4 rounded-md border border-gray-300 mb-6">
        ...
        <div className="flex flex-col md:flex-row md:space-x-52">
          ...
          <div className="w-full md:w-[30%] mb-2 ml-16">
            <label className={labelClass}>
              Suppliers Name<span className="text-red-400">*</span>
            </label>
            <CustomDropdown
              options={[
                { value: 'Category 1', label: 'John-87456564' },
                { value: 'Category 2', label: 'moin-98324244' },
                { value: 'Category 2', label: 'Moiz-23457890' },
              ]}
              value={category}
          ...
          <div className="w-full md:w-[30%] mb-2 ml-40">
            ...
            <CustomDropdown
              options={[
                { value: 'Category 1', label: 'Paid' },
                { value: 'Category 2', label: 'Unpaid' },
                { value: 'Category 2', label: 'partial' },
              ]}
              value={category}
              onChange={(value) => setCategory(typeof value === 'string' ? value : '')}
              className={inputClass}
